...
module.exports = (client, message) => {
    ...
    if (cmd && cmd.voiceChannel) {
        if (!message.member.voice.channel)
            return message.reply({ content: `❌ | Cậu phải call thì mình mới hát cho cậu nghe được chứ 😤!`, allowedMentions: { repliedUser: false } });

        if (message.guild.members.me.voice.channel && message.member.voice.channelId !== message.guild.members.me.voice.channelId)
            return message.reply({ content: `❌ | Cậu phải vô chung call room với mình chứ 😤!`, allowedMentions: { repliedUser: false } });
    }
